...
def filter_and_detrend(filename, start, end, polyorder=3, window=101, data=None):
    ...
    if end<saa_start:
        data['RATE'][start:end] -= (np.mean([data['RATE'][:start]]) + np.mean(data['RATE'][end:saa_start] + np.mean(data['RATE'][saa_end:])))/3
        data['RATE'][:start] -= savgol_filter(data['RATE'][:start], window, polyorder)
        data['RATE'][end:saa_start] -= savgol_filter(data['RATE'][end:saa_start], window, polyorder)
        data['RATE'][saa_end:] -= savgol_filter(data['RATE'][saa_end:], window, polyorder)
    elif start>saa_end:
        data['RATE'][start:end] -= (np.mean([data['RATE'][:saa_start]]) + np.mean(data['RATE'][saa_end:start] + np.mean(data['RATE'][end:])))/3
        data['RATE'][:saa_start] -= savgol_filter(data['RATE'][:saa_start], window, polyorder)
        data['RATE'][saa_end:start] -= savgol_filter(data['RATE'][saa_end:start], window, polyorder)
...
def quadratic_detrend(filename, start, end, polyorder=3, window=101, data=None):
    ...
    if end<saa_start:
        if start>background_window and (saa_start-end)>background_window:
            ...
            counts = np.concatenate((data['RATE'][start-background_window:start], grb, data['RATE'][end:saa_start]))
        ...
        if (start-saa_end)>background_window and (len(data.RATE)-end)>background_window:
            counts = np.concatenate((data['RATE'][start-background_window:start], grb, data['RATE'][end:end+background_window]))
            times = data['TIME'][start-background_window:end+background_window]
        elif (start-saa_end)<background_window and (len(data.RATE)-end)>background_window:
            counts = np.concatenate((data['RATE'][saa_end:start], grb, data['RATE'][end:end+background_window]))
...
def snr_rms(filename, start, end, polyorder=3):
    ...
    if end<saa_start:
        ...
        rms = np.sqrt(np.mean(data['RATE'][:saa_start]**2+data['RATE'][saa_end:start]**2))
        signal = np.max(data['RATE'][start:end])
        snr = signal/rms
    else:
        print('Inputted start and end times are not valid')
...
def snr_abs(filename, start, end, polyorder=3):
    ...
    if end<saa_start:
        ...
        abs = (np.mean(np.abs(data['RATE'][:saa_start]))+np.mean(np.abs(data['RATE'][saa_end:start])))/2
        signal = np.max(data['RATE'][start:end])
        snr = signal/abs
    else:
        print('Inputted start and end times are not valid')
...
def snr_counts(filename, start, end, polyorder=3, window=101):
    data, saa_start, saa_end = filter_and_detrend(filename, start, end, polyorder, window)
    ...
    if end<saa_start:
        noise = (np.sum(np.abs(data['RATE'][:start]))+np.sum(np.abs(data['RATE'][end:saa_start]))+np.sum(np.abs(data['RATE'][saa_end:])))/duration_noise
        
    elif start>saa_end:
        noise = (np.sum(np.abs(data['RATE'][:saa_start]))+np.sum(np.abs(data['RATE'][saa_end:start]))+np.sum(np.abs(data['RATE'][end:])))/duration_noise
    else:
        print('Inputted start and end times are not valid')
        snr = 0
